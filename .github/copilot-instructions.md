...
### Soft deletes
Transactions and some other records use `deleted_at` (nullable timestamp) rather than hard deletes. Always filter `deleted_at IS NULL` in queries.
...
2. **The relevant SQL script** — choose the right file:
   - New table or column → `sql_scripts/supabase_schema_create.sql` (creation) + a new `sql_scripts/supabase_<feature>.sql` migration for existing databases
